...
def cifar10(n_channel, pretrained=None):
    ...
    if pretrained is not None:
        model_dir, file_name = os.path.split(pretrained)
        m = model_zoo.load_url(model_urls["cifar10"], model_dir, file_name=file_name)
...
def cifar100(n_channel, pretrained=None):
    ...
    if pretrained is not None:
        model_dir, file_name = os.path.split(pretrained)
        m = model_zoo.load_url(model_urls["cifar100"], model_dir, file_name=file_name)
        state_dict = m.state_dict() if isinstance(m, nn.Module) else m
        assert isinstance(state_dict, (dict, OrderedDict)), type(state_dict)
        model.load_state_dict(state_dict)
    return model
